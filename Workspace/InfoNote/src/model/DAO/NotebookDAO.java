...
public class NotebookDAO {
	...
	public static List<Notebook> buscarTodos() {
		...
		try {
			...
			while (rs.next()) {
				notebooks.add(new Notebook(rs.getString("serialNote"), rs.getString("modelo"),
	...
	public static Notebook buscarPorSerial(String serial) {
		Notebook notebook = null;
		NotebookDAO noteDAO = new NotebookDAO();

		try {
			// Criação do insert
			String sql = "select * from notebook where serialnote = ?";
			...
			comando.setString(1, serial);

			ResultSet rs = comando.executeQuery();

			if (rs.next()) {
				notebook = new Notebook(rs.getString("serialNote"), rs.getString("modelo"), rs.getString("descricao"),
	...
	public static Notebook atualizar(String descricao, int estoque, double precoUnitario, String figura,
			String dataCadastro, String serialNote) {
		Notebook notebook = null;
		NotebookDAO noteDAO = new NotebookDAO();
